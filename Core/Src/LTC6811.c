...
void start_comm_with_LTC6811() {
    ...
    HAL_GPIO_WritePin(CSB_AFE_GPIO_Port, CSB_AFE_Pin,
            GPIO_PIN_RESET);
...
int broadcast_poll(uint16_t command) {
    ...
    start_comm_with_LTC6811();
    HAL_SPI_Transmit(&hspi1, message, 4, HAL_MAX_DELAY);
...
void broadcast_write(uint16_t command, uint16_t size, uint8_t *data) {
    ...
    HAL_SPI_Transmit(&hspi1, message, 4 + size + 2, HAL_MAX_DELAY);
...
int broadcast_read(uint16_t command, uint16_t size, uint8_t *data) {
    ...
    // Send command
    HAL_SPI_Transmit(&hspi1, command_message, 4, HAL_MAX_DELAY);
//  HAL_Delay(1);
...
// Receive data
    ...
    // Receive PEC
    HAL_SPI_Receive(&hspi1, data_PEC, 2, HAL_MAX_DELAY);
